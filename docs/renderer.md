...
| 438–545 | **Terminal status detection** constants | `IDLE_MS`, `STATE_LABEL`, `SELECT_FOOTER_RE`, `REVIEW_PROMPT_RE`, `MENU_PATTERNS`, `QUESTION_PATTERNS`, `ERROR_PATTERNS`, `CMD_MISSING_PATTERNS`, `stripAnsi`, `screenText`, `PROBE_MS` |
| 547–843 | Screen-parsed question cards + attention probe | `parseScreenQuestion`, `parseScreenReview`, `parsePlanScreenQuestion`, `parseCodexApproval`, `syncScreenQuestion`, `removeScreenQuestion`, `probeAttention`, `startAttentionProbe`, `stopAttentionProbe` |
...
| 1105–1134 | Prompt delivery to a PTY (bracketed paste) | `typePrompt`, `deliverPrompt` |
...
8. **Status detection is heuristic and version-pinned.** `MENU_PATTERNS`, `SELECT_FOOTER_RE`, the plan-menu regexes and the AskUserQuestion screen parsers were verified against specific Claude Code (v2.1.20x/2.1.21x) and codex-rs TUI output; when CLI wording drifts, these regexes are what to update. Buttons deliberately degrade to "answer in the terminal" rather than sending blind digits.
