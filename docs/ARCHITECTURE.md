...
5. **Security guards are load-bearing:** sandboxed renderer, regex-validated
   values spliced into PTY command strings (leading char must be alphanumeric so
   a value can't pose as a flag), `safeRef`/`--` argument-injection guards in
   git.js, no `innerHTML` for user/agent text. Additionally:
   - **`guardCwd` pins every `cwd`-taking IPC channel to a real hive directory**
     (`main.js`). The helper modules guard the *relative* path against the root;
     the root itself came from the renderer. See `docs/main-process.md` — wrap
     any new `cwd` channel.
   - **The renderer must never be able to make the OS run a file.**
     `image:saveTemp` allowlists image extensions; `files:open` refuses
     executable ones. Together those were a renderer→code-execution chain.
   - **`attach:stage` pins its source as well as its destination** — otherwise
     it copies credential files into a directory agent threads read.
6. **Concurrent writers:** the user's own agent threads edit this repo and the
   ...
   unreadable-vs-empty distinction in promptHistory.js exist for this;
   re-read files between your own edits too. **`boards.json` follows the same
   rule** (atomic `.tmp`+rename, corrupt files copied aside rather than
   overwritten) — it is the one file whose loss costs the user every hive.
   Renderer-side, `persist()` is debounced and chained so bursts collapse to one
   write and two writes never overlap.
