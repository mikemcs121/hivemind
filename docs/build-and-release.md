...
5. `publishRelease(cwd, version, onProgress)` (`build.js:220`) — verifies
   `dist/Hivemind ${version} portable.exe` exists, computes the exe's **SHA-256** and writes
   a `<exe>.sha256` sidecar (lowercase hex only), commits **only** `package.json`
   (pathspec-limited, so concurrently staged files from other Hivemind threads aren't
   swept in — `build.js:232`), pushes (both best-effort), then uploads **both** assets:
   `gh release create v${version} <exe> <exe>.sha256 --title "Hivemind v${version}"`. The
   `.sha256` sidecar is what lets the updater verify authenticity, not just byte size — keep
   uploading it.
...
| `files` | explicit allowlist (`package.json`, `*.js`, `src/**/*`, `models/**/*`, `build/icon.ico`, `node_modules/**/*`) | **Fail-closed.** This was `**/*`, which packed the *entire working directory* into `app.asar` — including gitignored local state (`.hivemind/` prompt history, plans, pasted attachments; `.claude/settings.local.json`). Those shipped inside published v0.1.14–v0.1.16 portable exes on the public repo and were extractable with `npx asar extract`. A blocklist can't fix this class of bug (the next `.env` still leaks), so only named paths are packed. Root `*.js` is deliberately a glob so new sidecar modules are picked up automatically, while non-code files at the root are not. **Adding a runtime file that lives outside these paths means adding it here, or it will be missing from the packaged app.** |
...
- **Asset**: the updater now **prefers** the asset whose name matches the exact target
  version (`Hivemind <ver> portable.exe`), only falling back to the loose `/portable/i`
  + ending `.exe` match (`updater.js:163`) when that exact-version asset isn't present.
  For the on-disk cleanup logic the canonical pattern is
...
- **`.sha256` sidecar asset** (`<exe>.sha256`, lowercase hex): the updater downloads it and
  **fails closed** (aborts the update, keeps the old exe) if it is missing or its digest
  doesn't match the downloaded exe. A release without this asset can no longer be installed —
  always publish both assets.
...
- **Check**: GETs `https://api.github.com/repos/mikemcs121/hivemind/releases/latest`
  (unauthenticated — this is why the repo must be public), 5s timeout, via `httpsGet`
  which follows redirects but **enforces HTTPS on each redirect**, caps the response body
  size, and has a response-stream error handler (`updater.js:80-96`). Compares `tag_name`
  against `app.getVersion()` with a numeric three-part semver compare (`isNewer`,
  `updater.js:28`).
...
- **Download/swap** (`updater.js:187-268`): downloads the asset to
  ...
  120s stall guard). Before promoting/launching it runs **two** integrity checks, both
  **fail closed**: (a) the byte size against the GitHub asset's reported `size` — a
  missing or zero `size` now also fails closed (previously that skipped the size check);
  and (b) a **SHA-256 authenticity** check — it downloads the release's `.sha256` sidecar
  asset and aborts unless the digest matches the downloaded exe (a missing sidecar also
  aborts). Only then does it rename `.part` to the final name and spawn the new exe
  detached. Deleting the old exe is gated on the new process **still running ~4.5s after
  spawn** — an immediate exit or spawn error is treated as a failed launch, keeps the old
  copy, and shows an error (previously the delete was gated merely on the `spawn` event
  firing). Cleanup of the old exe (locked until this process exits) is done by a
  self-deleting batch file in temp that retries `del` for ~30s (`ping` as the delay,
  because `timeout` needs console input; `updater.js:246-261`); the batch generator escapes
  `%`→`%%` and skips (logging) any path containing `"`, CR, or LF so a weird install path
  can't corrupt the batch. Then `app.quit()` with a 5s `app.exit(0)` failsafe because
  node-pty's ConPTY children can keep the process alive holding the file lock.
