---
name: release
description: Cut a Hivemind portable release by bumping the version, building, and publishing to GitHub releases so the auto-updater picks it up.
---

# Releasing Hivemind

Full background: `docs/build-and-release.md`. Run commands from the repository
root using any agent's shell tools. The in-app "Build portable copy" button
automates steps 2–6 (`build.js`, wired in `main.js`); use this manual flow when
releasing from an agent session. Publishing must be within the user's request;
loading this skill for inspection does not authorize a release.
...
1. **Preflight:** `gh auth status` must succeed before doing anything
   ...
   released. Preserve unrelated work from other threads.
